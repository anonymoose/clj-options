 ...
 */
public abstract class EquityOptionCalculator<T extends FDEngineAdapter> extends DividendVanillaOption {
    ...
    final private double dividendDiscount;
    final private double riskFreeDiscount;
    final private SimpleQuote spot;
    final private double forwardPrice;
     ...
     * @param v is the volatility
     * @param referenceDate is the settlement date
     ...
     * @param r is the risk free rate
     * @param v is the volatility
     * @param referenceDate is the expiration date
     * @param dates is a list of dates when dividends are expected to be paid
     * @param dividends is a list of dividends amounts (as a pure value) expected to be paid
    ...
    public EquityOptionCalculator(
        ...
        // prep other variables necessary to fill out the "more greeks" values.
        variance = this.stochProcess.blackVolatility().currentLink().blackVariance(exercise.lastDate(), ((PlainVanillaPayoff)payoff).strike());
        dividendDiscount = this.stochProcess.dividendYield().currentLink().discount(exercise.lastDate());
        riskFreeDiscount = this.stochProcess.riskFreeRate().currentLink().discount(exercise.lastDate());
        forwardPrice = spot.value() * dividendDiscount / riskFreeDiscount;
        DayCounter volatilityDayCounter = stochProcess.blackVolatility().currentLink().dayCounter();
        maturity = volatilityDayCounter.yearFraction(stochProcess.blackVolatility().currentLink().referenceDate(), exercise.lastDate());

        black = new BlackCalculator((PlainVanillaPayoff)payoff, forwardPrice, Math.sqrt(variance), riskFreeDiscount);
    }

    public double impliedVolatility(final double price) /* @ReadOnly */ {
        return impliedVolatility(price, stochProcess, 1.0e-4, 100, 1.0e-7, 4.0);
    }

    @Override
    public double delta() {
        return this.black.delta(spot.value());
    }

    @Override
    public double deltaForward() {
        return this.black.deltaForward();
    }

    @Override
    public double elasticity() {
        return this.black.elasticity(spot.value());
    }

    @Override
    public double gamma() {
        return this.black.gamma(spot.value());
    }

    @Override
    public double theta() {
        return this.black.theta(spot.value(), maturity);
    }

    @Override
    public double thetaPerDay() {
        return this.black.thetaPerDay(spot.value(), maturity);
    }

    @Override
    public double vega() {
        return this.black.vega(maturity);
    }

    @Override
    public double rho() {
        return this.black.rho(maturity);
    }

    @Override
    public double strikeSensitivity() {
        return this.black.strikeSensitivity();
    }

    @Override
    public double itmCashProbability() {
        return this.black.itmCashProbability();
    }
